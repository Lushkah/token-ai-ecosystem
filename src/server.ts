...
import { getLushkaMarket } from "./core/api/dex-screener/lushka-market.js";
import { getLushkaProsperity } from "./core/api/dex-screener/prosperity-engine.js";
...
async function getJson<T = unknown>(url: string): Promise<T> { const response = await fetch(url, { headers: { accept: "application/json" } }); if (!response.ok) throw new Error(`Upstream request failed: ${response.status}`); return response.json() as Promise<T>; }
...
app.get("/api/dex/token/:chainId/:address/market",async(req,res)=>{try{requireAddress(req.params.address);res.json(await getLushkaMarket(req.params.address,req.params.chainId));}catch(e){res.status(502).json({error:e instanceof Error?e.message:"Lushka market error"});}});
app.get("/api/dex/token/:chainId/:address/prosperity",async(req,res)=>{try{requireAddress(req.params.address);res.json(await getLushkaProsperity(req.params.address,req.params.chainId));}catch(e){res.status(502).json({error:e instanceof Error?e.message:"Lushka prosperity engine error"});}});
...
app.get("/api/token-intelligence/:chainId/:address",async(req,res)=>{try{requireAddress(req.params.address);const chainId=req.params.chainId,address=req.params.address;const [market,prosperity,abi]=await Promise.all([getLushkaMarket(address,chainId),getLushkaProsperity(address,chainId),process.env.ETHERSCAN_API_KEY?getJson(`${ETHERSCAN_BASE}?chainid=${encodeURIComponent(chainId)}&module=contract&action=getabi&address=${address}&apikey=${encodeURIComponent(process.env.ETHERSCAN_API_KEY)}`):Promise.resolve({status:"0",message:"Etherscan API key not configured"})]);res.json({chainId,address,market,prosperity,contract:abi});}catch(e){res.status(502).json({error:e instanceof Error?e.message:"Token intelligence error"});}});
